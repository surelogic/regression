...
import de.kvconnect.bundle.common.time.SleepUtils;

/**
 * An input stream which reads sequentially from multiple sources.
 * 
 * <p>
 * Initial Version: http://ostermiller.org/utils/src/ConcatInputStream.java.html
 * </p>
 * 
 * @see InputStream
 */
...
{
	private static final String MSG_INPUTSTREAM_CLOSED = "InputStream closed";
	 ...
	 */
	ArrayList<InputStream> inputStreamQueue = new ArrayList<InputStream>();

	/**
	 * A cache of the current inputStream from the inputStreamQueue to avoid unneeded access to the
	 * queue which must be synchronized.
	 */
	 ...
	 */
	boolean doneAddingInputStreams = false;

	/**
	 * Causes the addInputStream method to throw IllegalStateException and read() methods to return
	 * -1 (end of stream) when there is no more available data.
	 * <p>
	 ...
	 * @throws IllegalStateException if more inputStreams can't be added because
	 *         lastInputStreamAdded() has been called, close() has been called, or a constructor
	 *         with inputStream parameters was used.
	 */
	...
	/**
	 * Gets the current inputStream, looking at the next one in the list if the current one is null.
	 */
	...
	/**
	 * Indicate that we are done with the current inputStream and we should advance to the next
	 * inputStream.
	 */
	 ...
	 * <p>
	 * New sources should be added using the addInputStream() method. When all sources have been
	 * added the lastInputStreamAdded() should be called so that read methods can return -1 (end of
	 * stream).
	 * <p>
	...
	/**
	 * Reads the next byte of data from the underlying streams. The value byte is returned as an int
	 * in the range 0 to 255. If no byte is available because the end of the stream has been
	 * reached, the value -1 is returned. This method blocks until input data is available, the end
	 * of the stream is detected, or an exception is thrown.
	 * <p>
	 * If this class in not done accepting input streams and the end of the last known stream is
	 * reached, this method will block forever unless another thread adds an input stream or
	 * interrupts.
	 * 
	...
	public int read() throws IOException
	{
		if (closed)
		{
			throw new IOException(MSG_INPUTSTREAM_CLOSED);
		}
		int r = -1;
		while (r == -1)
		{
	...
	/**
	 * Reads some number of bytes from the underlying streams and stores them into the buffer array
	...
	/**
	 * Reads up to length bytes of data from the underlying streams into an array of bytes. An
	 * attempt is made to read as many as length bytes, but a smaller number may be read, possibly
	 * zero. The number of bytes actually read is returned as an integer.
	 * <p>
	 * If length is zero, then no bytes are read and 0 is returned; otherwise, there is an attempt
	 ...
	 * result from any of a number of conditions; reaching end of file before n bytes have been
	 * skipped is only one possibility. The actual number of bytes skipped is returned. If n is
