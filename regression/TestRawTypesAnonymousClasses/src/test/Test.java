...
public class Test {
  ...
  private final Object o = new B() {
    ...
    public void mm() {
      ...
      use(Test.this); // NOT_RAW -- Assumed to be, no way right now to annotate qualified receivers as otherwise, but perhaps should add in the future
      
      new Y() {
        private boolean g = use(this); // Raw(Y)
        // no way to refer to the outer object created by the first anonymous class
        private boolean ggg = use(Test.this); // NOT_RAW
        
        {
          use(this); // Raw(Y)
          // no way to refer to the outer object created by the first anonymous class
          use(Test.this); // NOT_RAW
        }
      };
  ...
  
  {
    ...
    final Object o = new B() {
      ...
      public void mm() {
        ...
        use(this); // NOT_RAW -- Assumed because there is no annotation to the contrary
        use(Test.this); // NOT_RAW -- Assumed to be, no way right now to annotate qualified receivers as otherwise, but perhaps should add in the future
  ...
  private class C extends B {
    private boolean f = use(this); // Raw(B)
    private boolean ff = use(Test.this); // NOT_RAW
    ...
    public void m() {
      ...
      new Y() {
        private boolean g = use(this); // Raw(Y)
        private boolean gg = use(C.this); // NOT_RAW
        private boolean ggg = use(Test.this); // NOT_RAW
  ...
  public void m() {
    ...
    final Object o = new B() {
      ...
      
      {
        ...
        /* Can call mm() from here because Test.this is NOT_RAW. */
        // mm(); // not an error
      }

      // Anonymous class should not break these
      private boolean f2 = use(this); // Raw(B)
      private boolean ff2 = use(Test.this); // NOT_RAW

      public void mm() {
        /* Should track: this(anon class), Test.this(Test), test(anon class of Y) */
        use(this); // NOT_RAW -- Assumed to be because it is not annotated otherwise
        use(Test.this); // NOT_RAW -- Assumed to be, no way right now to annotate qualified receivers as otherwise, but perhaps should add in the future
