...
public class ConcurrentQueueBlockingImpl<V> implements ConcurrentQueue<V> {
	...
	// we need to atomically update more than one location at a time in the
	// structure.
	// Short of a transactional memory implementation, we would either need a
	// complicated
	// implementation implementing recursive fixup, or something like the
	// Ladan-Mozes and
	// Shavit algorithm (see "An Optimistic Approach to Lock-Free FIFO Queues"
	// at
	// http://people.csail.mit.edu/edya/publications/publicationsAndPatents.htm)
	// that delays fixing up one direction in a double linked list. However,
	// that
	// algorithm does not consider general deletion, and I don't know whether
	// that
	// capability can be easily added or not.
	// Any of these approaches are quite complicated, and so we won't go there
	// yet.
...
//	@Aggregate("Instance into EntryRegion")
	...
	public V poll() {
		...
		// Once first is removed from the queue, it is invisible to other
		// threads,
